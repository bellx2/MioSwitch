...
class AppDelegate: UIResponder, UIApplicationDelegate {
	...
	func application(_ application: UIApplication, handleOpen url: URL) -> Bool {
		let regex = try! Regex("#access_token=(.*?)&")
		if let match = regex.firstMatch(url.absoluteString){
			saveSharedDefault(token: match.groups[0])
			NotificationCenter.default.post(name: NSNotification.Name("oAuthDone"), object: match.groups[0])
		}
